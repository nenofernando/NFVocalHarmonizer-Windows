...
NFVocalHarmonizerAudioProcessorEditor::NFVocalHarmonizerAudioProcessorEditor(NFVocalHarmonizerAudioProcessor& p)
    : AudioProcessorEditor(&p), processor(p), rail(p.apvts)
{
    setLookAndFeel(&look);
    ...
    for (auto* c : { static_cast<juce::Component*>(&rail), &trace, &inputMeter, &outputMeter,
                     &harmony, &formant, &humanize, &width, &mix }) addAndMakeVisible(c);
    ...
    analyzeButton.onClick = [safe = juce::Component::SafePointer<NFVocalHarmonizerAudioProcessorEditor>(this)] { if (safe != nullptr) safe->processor.resetKeyAnalysis(); };
    ...
    copyButton.onClick = [safe = juce::Component::SafePointer<NFVocalHarmonizerAudioProcessorEditor>(this)] {
        if (safe != nullptr) { safe->processor.captureSlot(safe->activeA); safe->processor.copySlot(safe->activeA); }
    };
    ...
    presetBox.onChange = [safe = juce::Component::SafePointer<NFVocalHarmonizerAudioProcessorEditor>(this)] {
        ...
        juce::String error;
        if (! safe->processor.presets.loadPreset(safe->presetBox.getText(), error)) safe->showError(error);
    };
    ...
    powerAtt = std::make_unique<ButtonAttachment>(p.apvts, nf::params::enabled, powerButton);
...
void NFVocalHarmonizerAudioProcessorEditor::timerCallback()
{
    ...
    const auto pitch = processor.getPitchEstimate();
    ...
    const bool autoMode = processor.apvts.getRawParameterValue(nf::params::autoKey)->load() > 0.5f;
    if (autoMode)
    {
        const auto detected = processor.getDetectedScale(); root = detected.root; scale = detected.type;
...
void NFVocalHarmonizerAudioProcessorEditor::refreshPresetList(const juce::String& select)
{
    ...
    presetBox.addItemList(processor.presets.listPresets(), 1);
...
{
    const auto initial = processor.presets.getPresetDirectory().getChildFile("My Harmony.nfhpreset");
    ...
    fileChooser->launchAsync(juce::FileBrowserComponent::saveMode | juce::FileBrowserComponent::canSelectFiles
        ...
        [safe = juce::Component::SafePointer<NFVocalHarmonizerAudioProcessorEditor>(this)](const juce::FileChooser& chooser)
        {
            ...
            juce::String error;
            if (! safe->processor.presets.savePresetToFile(file, error)) safe->showError(error);
...
void NFVocalHarmonizerAudioProcessorEditor::switchAB(bool useA)
{
    ...
    processor.captureSlot(activeA); processor.restoreSlot(useA); activeA = useA;
}
